...
class RequestApi {
  ...
  Future<dynamic> request({bool getResponseBytes = false}) async {
    ...
    http.MultipartRequest request = MultipartRequest(method, uri, onProgress: (int? bytes, int? totalBytes) {
      ...
      final rOP = Provider.of<UploadRequestProgressProvider>(currentContext_!, listen: false);
  ...
  Future<dynamic> httpSendRequest() async {
    ...
    try {
      request.headers.addAll({
        ...
        "languageid": SharedPref.getLanguage() == "en"?"0":"1",
      ...
      final int? contentLength = int.tryParse(response.headers["content-length"] ?? "");
      if(contentLength != null){
        if(currentContext_ == null) return;
        final drP = Provider.of<DownloadRequestProgressProvider>(currentContext_!, listen: false);
        ...
        response.stream.listen(
          (chunk) {
            ...
            drP.updateRequestOnProgress(bytes: receivedBytes,totalBytes: contentLength);
          },
          onDone: ()=> drP.updateRequestOnProgress(bytes: contentLength,totalBytes: contentLength),
          onError: (e)=> drP.updateRequestOnProgress(bytes: 0,totalBytes: 0),
          cancelOnError: true,
        );
        while(bytes.length != contentLength){
          await Future.delayed(const Duration(milliseconds: 100));
        }
        if(getResponseBytes) responseBytes = Uint8List.fromList(bytes);
        if(!getResponseBytes) responseText = utf8.decode(bytes);
      }else{
